...
const ChallengeDetail = () => {
	...
	const getChallengeData = async () => {
		try {
			const url = isLogin
				? `${process.env.REACT_APP_SERVER_URL}/api/challenges/details/${challengeId}?memberId=${memberId}`
	...
	const handleOrgClick = async () => {
		try {
			const response = await axios.post(
				`${process.env.REACT_APP_SERVER_URL}/api/challengers`,
				{
					challengeId,
					memberId,
				},
	...
	return (
		<ChallengeDetailContainer>
			<TitleHeader title={challengeData.title} />
			<ChallengeDetailWrapper>
				<StyledImg
					src={random[Math.floor(Math.random() * random.length)]}
					alt={challengeData.title}
				/>
				{/* <StyledImg src={running} alt={challengeData.title} /> */}
				<StyledH1>{challengeData.title}</StyledH1>
				<InfoTag label={`${challengeData.challengerCount}명`} width="4.5rem" />
				<SeparateLine></SeparateLine>
				<StyledUl>
					<li>
						기간
						<div>
							{challengeData.startAt} - {challengeData.endAt}
						</div>
					</li>
					<li>
						빈도<div>{challengeData.frequency}</div>
					</li>
					<li>
						인증시간
						<div>
							{challengeData.snapshotStartAt} - {challengeData.snapshotEndAt}
						</div>
					</li>
				</StyledUl>
				<SeparateLine></SeparateLine>
