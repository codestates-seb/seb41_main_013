...
public class CommentController {
    ...
    public ResponseEntity postComment(@Valid @RequestBody CommentPostDto commentPostDto){
        Comment response = commentService.createComment(commentMapper.commentPostDtoToComment(commentPostDto));
    ...
    @GetMapping("/{board-id}")
    public ResponseEntity getComments(@PathVariable("board-id")
            @RequestParam("page") @Positive int page,
            @RequestParam("size") @Positive int size){
       // List<Comment> response = commentService.findComments(boardId, page-1, size).getContent();
